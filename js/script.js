// ============================================================
// script.js — Site Romântico
// Versão com Firebase Realtime Database
// Remetente edita → gera link → destinatário vê sem edição
// ============================================================
...
// ===================== MODO DO SITE =====================
// Se a URL tiver ?id=xxx → modo visualização (destinatário)
// Se não tiver → modo edição (remetente)
...
let siteData = {
  ...
  skyLng: '-49.0000',
  moments: [
    { date: '01 de Janeiro de 2001', desc: 'Onde tudo começou a começar — nosso primeiro encontro especial.', img: null },
...
function goPage(pg) {
  ...
  if (targetId === 'pageAdmin') {
    ...
    // Só mostra o botão de edição se NÃO for modo visualização
...
function initFakeMap() {
  ...
  if (!canvas) {
    ...
    grad.className = 'map-overlay-gradient';
    container.appendChild(grad);

    const badge = document.createElement('div');
    badge.className = 'map-coords-badge';
    badge.id = 'mapCoordsBadge';
    container.appendChild(badge);

    const btn = document.createElement('a');
    btn.id = 'mapOpenBtn';
    btn.className = 'map-open-btn';
    btn.target = '_blank';
    btn.rel = 'noopener noreferrer';
    btn.innerHTML = '🗺️ Abrir no Google Maps';
  ...
  for (let i = 0; i < 24; i++) {
    blocks.push({
      ...
      w: rand() * 80 + 30,
      h: rand() * 60 + 20,
      color: rand() > 0.5 ? '#1e2a1e' : '#182218'
    });
  }

  const roads = [];
  for (let i = 0; i < 6; i++) {
    roads.push({ type: 'h', y: rand() * H });
    roads.push({ type: 'v', x: rand() * W });
  }

  function drawFakeMap() {
    fakeMapAnimFrame = requestAnimationFrame(drawFakeMap);
    t++;
    ...
    targetX += panDX;
    targetY += panDY;
    if (Math.abs(targetX) > 120) panDX *= -1;
    if (Math.abs(targetY) > 80) panDY *= -1;
    ...
    ctx.strokeStyle = 'rgba(60,90,60,0.35)';
    ctx.lineWidth = 1;
    const gx = ((ox % TILE) + TILE) % TILE;
    const gy = ((oy % TILE) + TILE) % TILE;
    for (let x = -gx; x < W + TILE; x += TILE) {
    ...
    roads.forEach(r => {
      ctx.beginPath();
      if (r.type === 'h') { ctx.moveTo(0, r.y + oy * 0.5); ctx.lineTo(W, r.y + oy * 0.5); }
      else { ctx.moveTo(r.x + ox * 0.5, 0); ctx.lineTo(r.x + ox * 0.5, H); }
      ctx.stroke();
    });
    ...
    blocks.forEach(b => {
      ...
      ctx.beginPath();
      ctx.roundRect(b.x + ox * 0.7, b.y + oy * 0.7, b.w, b.h, 3);
      ctx.fill();
    });
    ...
    ctx.beginPath();
    ctx.arc(pinX, pinY, 18 + pulse * 8, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(194,24,91,${0.15 + pulse * 0.1})`;
    ctx.fill();
    ctx.beginPath();
    ctx.arc(pinX, pinY, 10, 0, Math.PI * 2);
    ctx.fillStyle = 'var(--color-primary, #c2185b)';
    ...
    if (p && p.name) {
      ...
      const textW = ctx.measureText(p.name).width + 20;
      ctx.fillStyle = 'rgba(0,0,0,0.7)';
      ctx.beginPath();
      ctx.roundRect(pinX - textW/2, pinY - 42, textW, 22, 6);
      ctx.fill();
      ctx.fillStyle = '#fff';
      ctx.fillText(p.name, pinX, pinY - 26);
      ctx.textAlign = 'left';
    }
  }
...
// ===================== PÁGINA DO CÉU =====================
function renderSky() {
  const name = `${siteData.name1} e ${siteData.name2}`;
  document.getElementById('skyCupleName').textContent = name;
  ...
  document.getElementById('skyMeta').innerHTML = `
    ${siteData.skyCity}, ${siteData.skyState}, ${siteData.skyCountry}<br>
    ${dateStr} - ${siteData.skyTime}<br>
    ${latStr} ${lngStr}
  `;
...
function drawSkyCanvas() {
  ...
  for (let i = 0; i < 120; i++) {
    ...
    const size = rand() * 2 + 0.3;
    stars.push({ x, y, size });
    ctx.beginPath();
    ctx.arc(x, y, size, 0, Math.PI*2);
    ctx.fillStyle = `rgba(255,255,255,${rand()*0.6+0.4})`;
    ctx.fill();
  }

  const constellationStars = stars.slice(0, 20);
  const groups = [[0,3,7],[3,8,12],[12,15,18],[7,10,14,19]];
  ctx.strokeStyle = 'rgba(255,255,255,0.4)';
  ctx.lineWidth = 0.8;
  groups.forEach(g => {
    for (let i = 0; i < g.length-1; i++) {
      const s1 = constellationStars[g[i]];
      const s2 = constellationStars[g[i+1]];
      ctx.beginPath();
    ...
    g.forEach(idx => {
      const s = constellationStars[idx];
      ctx.beginPath();
...
function renderMomentsAdmin() {
  ...
  siteData.moments.forEach((m, i) => {
    ...
    div.innerHTML = `
      <button class="moment-remove" onclick="removeMoment(${i})">×</button>
      <div class="form-group"><label class="form-label">Data / Período</label>
        <input class="form-input" value="${m.date}" onchange="siteData.moments[${i}].date=this.value" placeholder="Ex: 12 de Março de 2007">
      </div>
      <div class="form-group"><label class="form-label">Descrição</label>
        <input class="form-input" value="${m.desc}" onchange="siteData.moments[${i}].desc=this.value" placeholder="O que aconteceu?">
      </div>
      <div class="form-group"><label class="form-label">Foto</label>
        <div class="upload-area" onclick="document.getElementById('mom-img-${i}').click()">
          <input type="file" id="mom-img-${i}" accept="image/*" onchange="loadMomentImg(this,${i})">
...
function renderPlacesAdmin() {
  ...
  siteData.places.forEach((p, i) => {
    ...
    div.innerHTML = `
      <button class="moment-remove" onclick="removePlace(${i})">×</button>
      <div class="form-grid2">
        <div class="form-group"><label class="form-label">Nome do local</label>
          <input class="form-input" value="${p.name}" onchange="siteData.places[${i}].name=this.value">
        </div>
        <div class="form-group"><label class="form-label">Data</label>
          <input class="form-input" value="${p.date}" onchange="siteData.places[${i}].date=this.value">
        </div>
      </div>
      <div class="form-grid3">
        <div class="form-group"><label class="form-label">Cidade</label>
          <input class="form-input" value="${p.city}" onchange="siteData.places[${i}].city=this.value">
        </div>
        <div class="form-group"><label class="form-label">Estado</label>
          <input class="form-input" value="${p.state}" onchange="siteData.places[${i}].state=this.value">
        </div>
        <div class="form-group"><label class="form-label">País</label>
          <input class="form-input" value="${p.country}" onchange="siteData.places[${i}].country=this.value">
        </div>
      </div>
      <div class="form-group"><label class="form-label">Texto / Memória</label>
        <input class="form-input" value="${p.quote}" onchange="siteData.places[${i}].quote=this.value">
      </div>
      <div class="form-grid2">
        <div class="form-group"><label class="form-label">Latitude</label>
          <input class="form-input" value="${p.lat}" onchange="siteData.places[${i}].lat=this.value" placeholder="-12.0000">
        </div>
        <div class="form-group"><label class="form-label">Longitude</label>
          <input class="form-input" value="${p.lng}" onchange="siteData.places[${i}].lng=this.value" placeholder="-49.0000">
        </div>
      </div>
      <div class="form-group"><label class="form-label">Foto</label>
        <div class="upload-area" onclick="document.getElementById('place-img-${i}').click()">
          <input type="file" id="place-img-${i}" accept="image/*" onchange="loadPlaceImg(this,${i})">
...
function addMomentAdmin() {
  siteData.moments.push({ date: '', desc: '', img: null });
  renderMomentsAdmin();
}
function removeMoment(i) {
  if (siteData.moments.length <= 1) { showToast('Mínimo de 1 momento obrigatório'); return; }
  siteData.moments.splice(i, 1);
  renderMomentsAdmin();
}
function addPlaceAdmin() {
  siteData.places.push({ name: '', date: '', city: '', state: '', country: '', quote: '', lat: '', lng: '', img: null });
  renderPlacesAdmin();
}
function removePlace(i) {
  if (siteData.places.length <= 2) { showToast('Mínimo de 2 lugares obrigatórios'); return; }
  siteData.places.splice(i, 1);
  renderPlacesAdmin();
}
...
function previewImage(input, previewId, targetId) {
  const file = input.files[0];
  if (!file) return;
  const reader = new FileReader();
  reader.onload = e => {
...
function loadMusic(input) {
  ...
  reader.onload = function(e) {
    siteData.songUrl = e.target.result;
...
function loadSongArt(input) {
  ...
  const reader = new FileReader();
  reader.onload = e => {
    siteData.songArtImg = e.target.result;
    updateSongArtDisplay();
  };
  reader.readAsDataURL(file);
}
...
function updateSongArtDisplay() {
  ...
  if (siteData.songArtImg) {
    if (imgEl) {
      imgEl.src = siteData.songArtImg;
    } else {
      const img = document.createElement('img');
      img.src = siteData.songArtImg;
      img.style.cssText = 'width:100%;height:100%;object-fit:cover;border-radius:0.6rem;';
      art.childNodes.forEach(n => { if (n.nodeType === 3) n.remove(); });
      const overlay = art.querySelector('.player-art-overlay');
      art.insertBefore(img, overlay);
    }
  }
}
...
function saveAll() {
  ...
  applyDataToPage1();
  showToast('✅ Salvo! Agora clique em "Gerar Link" para criar o link do destinatário.');
}

// ===================== GERAR LINK (FIREBASE) =====================
async function gerarLink() {
  // Lê os campos do admin antes de salvar
  saveAll();
...
async function carregarDoFirebase(id) {
  ...
  try {
    ...
    showToast('❌ Erro ao carregar. Tente novamente.');
    console.error(err);
  }
}

// ===================== APLICAR DADOS NA PÁGINA 1 =====================
function applyDataToPage1() {
  const coupleName = `${siteData.name1} e ${siteData.name2}`;
  document.getElementById('p1-couple-name').textContent = coupleName;
  ...
  const preview = lines.slice(0,2).join('<br>') + (lines.length > 2 ? '...' : '');
  document.getElementById('mensagemPreview').innerHTML = preview;
  ...
  if (siteData.songUrl) audio.src = siteData.songUrl;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (MODO_VISUALIZACAO) {
    // Destinatário: carrega os dados do Firebase pelo ID da URL
    await carregarDoFirebase(SITE_ID);
  } else {
    // Remetente: carrega do localStorage normalmente
    try {
